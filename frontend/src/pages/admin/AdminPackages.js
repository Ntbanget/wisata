...
const AdminPackages = () => {
  ...
  const filteredPackages = packages.filter(pkg => {
    return searchTerm === '' || 
      ...
      pkg.city_name?.toLowerCase().includes(searchTerm.toLowerCase());
  });
    ...
    <div className="p-8">
      <div className="mb-8 flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-2">Manage travel packages</p>
        </div>
            ...
            <div key={pkg.id} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
              <div className="p-4">
                <div className="flex items-start justify-between mb-2">
                  <h3 className="font-semibold text-gray-900">{pkg.name || `Package #${pkg.id}`}</h3>
                  <span className="px-2 py-1 text-xs font-medium rounded bg-indigo-100 text-indigo-700">
                    {pkg.city_name || 'Unknown City'}
                  </span>
                </div>
                <div className="space-y-2 text-sm text-gray-600 mb-2">
                  <p><span className="font-medium">Nights:</span> {pkg.nights || 1}</p>
                  <p><span className="font-medium">Destinations:</span> {pkg.tourist_places?.length || 0}</p>
                </div>
